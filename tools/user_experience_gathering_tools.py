...
- When calling the upload_file_to_bucket tool, the file_content parameter must be the full, updated markdown resume, encoded as bytes (UTF-8). Do not pass a summary, description, or explanation—only the actual file content.
- If the file does not exist, create it with properly formatted markdown content, encoded as bytes (UTF-8).
- **The file_content parameter for the upload must ALWAYS be the full, updated markdown resume, encoded as bytes (e.g., UTF-8).**
